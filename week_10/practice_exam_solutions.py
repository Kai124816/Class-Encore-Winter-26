...
def score(word: str, values: dict[str, int]) -> int:
    ...
    >>> score("this is easy", {'i': 5, 'e': 2})
    12
    >>> score("I struck out", {'w': 8, 'x': 9, 'q': 10})
    0
    """
    acc = 0
...
def match_at(m: list[int], n: list[int]) -> int:
    ...
    >>> match_at([1, 2, 3], [3, 2, 1]) 
    1
    >>> match_at([1, 2, 5, 3], [5, 3, 5]) 
    2
    >>> match_at([1, 2, 5, 3], [3, 5, 1]) 
    -1
    """
    shorter_len = min(len(m), len(n))
...
def most(collection: list[tuple[str, int]]) -> str:
    ...
    has the greatest total count (sum of all the counts)? (All counts are positive.)
    ...
    max_key = collection[0][0]

    for key in count_dict.keys():
        if count_dict[key] > count_dict[max_key]:
            max_key = key
    return max_key
...
# ------------------------------------------------------------------------------
ANSWER_4 = "Lists are mutable; tuples are immutable"
...
# ------------------------------------------------------------------------------
ANSWER_7 = "The accumulator on line A should be count += 1"
...
# ------------------------------------------------------------------------------
ANSWER_10 = "Sorts the list by word length from shortest to longest"
